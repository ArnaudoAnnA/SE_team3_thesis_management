...
const acceptRejectSTR = async (id, accept) => {
  ...
  try {
    ...
    if (!res.error) {
      ...
      //If the user tries to accept/reject an approved/rejected request, return error
      if (res.STR.approved !== null ) return {status: 400, error: "Thesis request already approved/rejected"}

      //If approved, update the approvalDate field with the current date, otherwise leave it null
      if (accept){
